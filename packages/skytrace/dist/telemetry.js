...
exports.isEnabled = exports.notice = exports.init = void 0;
...
const noop = () => { };
const notice = () => {
    console.log('Anonymized telemetry is on. Learn more: https://skytrace.dev/docs/resources/core/telemetry.html');
};
...
const init = () => {
    ...
    const debugEnabled = typeof process.env.SKYTRACE_TELEMETRY_DEBUG !== 'undefined';
    ...
    const capture = (client) => {
        return (event, data = {}) => {
            ...
            if (debugEnabled) {
                console.log(chalk.yellow(`Telemetry data: ${JSON.stringify(eventPayload)}`));
            }
            try {
                debug({ eventPayload });
